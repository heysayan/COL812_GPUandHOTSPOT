...
blist_t *new_blist(int idx, double occupancy, double res, double res_l, double specificHeat,int first,int do_detailed_3D, double cw, double ch, double thickness, int has_lateral)
{
  ...
  if(first && do_detailed_3D){
      if(occupancy >= OCCUPANCY_THRESHOLD){
          ptr->lock=TRUE;
          if (has_lateral) {
            ptr->rx =  getr(1/res_l, cw, ch * thickness);	
            ptr->ry =  getr(1/res_l, ch, cw * thickness);
          }
          else {
            ptr->rx =  LARGENUM;	
            ptr->ry =  LARGENUM;
          ...
          ptr->lock=FALSE;
          if (has_lateral) {
            ptr->rx = 1 / ((1 / getr(1 / res_l, cw, ch * thickness)) * occupancy);
            ptr->ry = 1 / ((1 / getr(1 / res_l, ch, cw * thickness)) * occupancy);
          }
          else {
            ptr->rx =  LARGENUM;	
            ptr->ry =  LARGENUM;
...
void blist_append(blist_t *head, int idx, double occupancy,double res, double res_l, double specificHeat,int first, int do_detailed_3D,double cw, double ch, double thickness, int has_lateral)
{
  /*BU_3D: 
  ...
  if(do_detailed_3D && (head->lock!=TRUE)){
      if(occupancy >= OCCUPANCY_THRESHOLD){
          head->lock=TRUE;
          if (has_lateral) {
              head->rx =  getr(1/res_l, cw, ch * thickness);
              head->ry =  getr(1/res_l, ch, cw * thickness);
          }
          else {
              head->rx = LARGENUM;
              head->ry = LARGENUM;
          ...
          if (has_lateral) {
              head->rx = 1/((1/head->rx) + ((1 / getr(1 / res_l, cw, ch * thickness)) * occupancy));
              head->ry = 1/((1/head->ry) + ((1 / getr(1 / res_l, ch, cw * thickness)) * occupancy));
          }
          else {
              head->rx = LARGENUM;
              head->ry = LARGENUM;
  ...
  tail->next = new_blist(idx, occupancy, res, res, specificHeat, first, do_detailed_3D, cw, ch, thickness, has_lateral);
}
      ...
      for(i=i1; i < i2; i++) {
          for(j=j1; j < j2; j++) {
              ...
              if ((i > i1) && (i < i2-1) && (j > j1) && (j < j2-1)) {
                  ...
                  if (!layer->b2gmap[i][j]){
                      layer->b2gmap[i][j] = new_blist(u, 1.0,res,res,sh,1,model->config.detailed_3D_used,cw,ch,layer->thickness,layer->has_lateral);
                       ...
                       * be sharing it */
                      blist_append(layer->b2gmap[i][j], u, 1.0,res,res,sh,1,model->config.detailed_3D_used,cw,ch,layer->thickness,layer->has_lateral);
                  ...
                  /* first unit in the list	*/
                  if (!layer->b2gmap[i][j]){
                      layer->b2gmap[i][j] = new_blist(u, occupancy,res,res,sh,1,model->config.detailed_3D_used,cw,ch,layer->thickness,layer->has_lateral);
                      layer->b2gmap[i][j]->hasRes = TRUE;//BU_3D assign hasRes to the b2gdata structure
                      layer->b2gmap[i][j]->hasCap = layer->flp->units[u].hasSh;//BU_3D assign hasSh to the b2gdata structure
                  }
                  else
                    /* append at the end */
                    blist_append(layer->b2gmap[i][j], u, occupancy,res,res,sh,0,model->config.detailed_3D_used,cw,ch,layer->thickness,layer->has_lateral);
              }
          }
      }
  }
