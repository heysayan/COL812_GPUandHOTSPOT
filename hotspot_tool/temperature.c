...
double bank_state_array[128];

void read_bank_state_array()
{
    FILE *myFile;
    myFile = fopen("BankStateData.txt", "r");
    //read file into array
    int i;
    if (myFile == NULL){
        printf("Error Reading File bankStateData.txt\n");
        exit (0);
    }
    for (i = 0; i < 128; i++){
        fscanf(myFile, "%lf,", &bank_state_array[i] );
    }
    /*for (i = 0; i < 128; i++){
        printf("Number is: %lf\n\n", bank_state_array[i]);
    }*/
...
//	leakage_power = 0.272;					// At 353 (80)
	...
	// Some layers with memory banks end up here.
	if (unit != NULL && strstr(unit->name, "B_") != NULL)
	{
		char * p = unit->name;
		...
		if (model->config->leakage_used) { // if considering leakage-temperature loop
			...
			for (leak_iter=0;(!leak_convg_true)&&(leak_iter<=LEAKAGE_MAX_ITER);leak_iter++){
				for(i=0; i < n; i++) {
					...
					power_new[i] = power[i] + calc_leakage(model->config->leakage_mode,blk_height,blk_width,temp[i], &model->block->flp->units[i], model->bank_modes);
					...
					if(model->grid->layers[k].has_power)
						for(j=0; j < model->grid->layers[k].flp->n_units; j++) {
							...
							if (k==3){ 	// Layer0 : In 3Dmem is an SRAM layer its leakage model is different.
									...
									blk_id = ((k-5)/2)*16 + j;
									if(bank_state_array[blk_id] == 0) {
										power_new[base+j] = 0.17 * calc_leakage(model->config->leakage_mode,blk_height,blk_width,temp_first_time[base+j], &model->grid->layers[k].flp->units[j], model->bank_modes);
                                        //printf("[Naman][hotspot] In compute temperature blk_id OFF %d\n", blk_id);
										//power_new[base+j] = 0.17 * calc_leakage(model->config->leakage_mode,blk_height,blk_width,temp_first_time[base+j]);
				 					}
                                    else {
                                        ...
                                        //printf("[Naman][hotspot] In compute temperature blk_id ON %d\n", blk_id);
                                    }
							}
                            /*
									if (leakage[j] == 0)
...
//	leakage_power = 0.2156328354;			// At 353 (80)
	...
	// Look up memory bank mode and scale the leakage power.
	if (unit != NULL && strstr(unit->name, "B_") != NULL)
	{
		char * p = unit->name;
		int bank_id = strtol(unit->name+2, NULL, 10);
